...
model_settings_pattern = re.compile(
    '\d+-\d+-ctm-I(?P<iteration>\d+)-S(?P<snapshot>\d+)-K(?P<topic>\d+)-am(?P<alpha_mu>[\d\.]+)-as(?P<alpha_sigma>[\d\.]+)-ab(?P<alpha_beta>[\d\.]+)'
)
...
def parse_args():
    ...
    parser.set_defaults(
        model_directory=None, snapshot_index=-1, output_directory=None,
        training_iterations=-1, snapshot_interval=-1)
    # parameter set 1
    ...
    # help="input directory [None]");
    parser.add_option(
        "--model_directory", type="string", dest="model_directory",
        help="model directory [None]")
    parser.add_option(
        "--snapshot_index", type="int", dest="snapshot_index",
    ...
    # parameter set 2
    parser.add_option(
        "--output_directory", type="string", dest="output_directory",
    ...
    parser.add_option(
        "--training_iterations", type="int", dest="training_iterations",
        help="number of training iterations [-1]")
    parser.add_option(
        "--snapshot_interval", type="int", dest="snapshot_interval",
...
def main():
    ...
    model_snapshot_file_path = os.path.join(
        model_directory, "model-%d" % snapshot_index)
    if not os.path.exists(model_snapshot_file_path):
        sys.stderr.write(
            "error: model snapshot file unfound %s...\n" %
            (model_snapshot_file_path))
        return
    ctm_inferencer = pickle.load(open(model_snapshot_file_path, "rb"))
    print(
        'successfully load model snapshot %s...' %
        (os.path.join(model_directory, "model-%d" % snapshot_index)))
    ...
    shutil.copy(
        model_snapshot_file_path,
        os.path.join(output_directory, "model-" + str(snapshot_index)))
    shutil.copy(
        model_snapshot_file_path,
        os.path.join(output_directory, "exp_beta-" + str(snapshot_index)))
    ...
    for iteration in range(snapshot_index, training_iterations):
        ...
        if ((ctm_inferencer._counter) % snapshot_interval == 0):
            ctm_inferencer.export_beta(
                os.path.join(
                    output_directory,
                    'exp_beta-' + str(ctm_inferencer._counter)))
            model_snapshot_path = os.path.join(
                output_directory, 'model-' + str(ctm_inferencer._counter))
