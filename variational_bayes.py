...
class Process_E_Step_Queue(multiprocessing.Process):
    def __init__(self,
        ...
        if result_sufficient_statistics_queue != None:
            self._E_log_prob_eta = self._E_log_eta - scipy.misc.logsumexp(self._E_log_eta, axis=1)[:, numpy.newaxis]
...
class VariationalBayes(Inferencer):
    ...
    def format_result_queues(self, number_of_documents, result_doc_parameter_queue, result_log_likelihood_queue, result_sufficient_statistics_queue=None):  
        # initialize a D-by-K matrix lambda and nu_square values
        lambda_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_mu[numpy.newaxis, :];
        nu_square_values = numpy.zeros((number_of_documents, self._number_of_topics))  # + self._alpha_sigma[numpy.newaxis, :];
    ...
    def e_step(self,
        ...
        for doc_id in numpy.random.permutation(number_of_documents):
            ...
            if parsed_corpus != None:
                # compute the phi terms
                words_log_likelihood += numpy.sum(numpy.exp(log_phi + numpy.log(term_counts)) * E_log_prob_eta[:, term_ids]);
    ...
    def optimize_hyperparameter(self):
        ...
        if self._diagonal_covariance_matrix:
            self._alpha_sigma = numpy.mean(self._nu_square + (self._lambda - self._alpha_mu[numpy.newaxis, :]) ** 2, axis=0);
    ...
    def hessian_free_lambda(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_lambda = self.conjugate_gradient_delta_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_lambda(self,
        ...
        if precondition_hessian_matrix:
            hessian_lambda = self.second_derivative_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        ...
        r_vector -= self.hessian_damping_direction_approximation_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, delta_doc_lambda)
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, p_vector);
    ...
    def first_derivative_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count):
        ...
        if self._diagonal_covariance_matrix:
            ...
            first_derivative_lambda = numpy.dot((self._alpha_mu - doc_lambda[numpy.newaxis, :]), self._alpha_sigma_inv)[0, :]
    ...
    def hessian_direction_approximation_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        ...
        hessian_direction_lambda = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
        
        if self._diagonal_covariance_matrix:
            hessian_direction_lambda = -direction_vector * epsilon / self._alpha_sigma;
        else:
            hessian_direction_lambda += -numpy.dot(direction_vector[numpy.newaxis, :] * epsilon, self._alpha_sigma_inv)[0, :]
    ...
    def hessian_damping_direction_approximation_lambda(self, doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_lambda(doc_lambda + direction_vector, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
        damping_factor_numerator -= self.function_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_lambda(doc_lambda, doc_nu_square, doc_zeta_factor, sum_phi, total_word_count);
    ...
    def hessian_free_nu_square(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_nu_square = self.conjugate_gradient_delta_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_nu_square(self,
        ...
        r_vector -= self.hessian_damping_direction_approximation_nu_square(doc_lambda, doc_nu_square_copy, doc_zeta_factor, total_word_count, delta_doc_nu_square);
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_nu_square(doc_lambda, doc_nu_square_copy, doc_zeta_factor, total_word_count, p_vector);
    ...
    def hessian_direction_approximation_nu_square(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        ...
        hessian_direction_nu_square = total_word_count * numpy.exp(-log_exp_over_doc_zeta_b) * (1 - numpy.exp(log_exp_over_doc_zeta_b - log_exp_over_doc_zeta_a))
    ...
    def hessian_damping_direction_approximation_nu_square(self, doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        damping_factor_numerator = self.function_nu_square(doc_lambda, doc_nu_square + direction_vector, doc_zeta_factor, total_word_count);
        damping_factor_numerator -= self.function_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count);
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
        damping_factor_denominator_temp += 0.5 * hessian_direction_approximation;
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
    ...
    def hessian_free_nu_square_in_log_space(self,
        ...
        for hessian_free_iteration_index in xrange(hessian_free_iteration):
            delta_doc_log_nu_square = self.conjugate_gradient_delta_log_nu_square(doc_lambda, doc_nu_square, doc_zeta_factor, total_word_count, self._number_of_topics);
    ...
    def conjugate_gradient_delta_log_nu_square(self,
        ...
        if precondition_hessian_matrix:
            hessian_log_nu_square = self.second_derivative_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        ...
        r_vector = -self.first_derivative_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        ...
        r_vector -= self.hessian_damping_direction_approximation_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, delta_doc_log_nu_square);
        ...
        for conjugate_gradient_iteration_index in xrange(conjugate_gradient_iteration):
            ...
            assert numpy.all(numpy.isfinite(r_vector)), (conjugate_gradient_iteration_index, r_vector, doc_nu_square, -self.first_derivative_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count), -self.hessian_direction_approximation_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, delta_doc_log_nu_square))
            ...
            hessian_p_vector = self.hessian_damping_direction_approximation_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, p_vector);
    ...
    def second_derivative_log_nu_square(self, doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count):
        ...
        second_derivative_log_nu_square -= 0.5 * total_word_count * exp_over_doc_zeta * exp_doc_log_nu_square * (1 + 0.5 * exp_doc_log_nu_square);
        
        return second_derivative_log_nu_square;    

    def hessian_direction_approximation_log_nu_square(self, doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector, epsilon=1e-6):
        assert doc_lambda.shape == (self._number_of_topics,)
        assert doc_log_nu_square.shape == (self._number_of_topics,)
        assert doc_zeta_factor.shape == (self._number_of_topics, self._number_of_topics)
        assert direction_vector.shape == (self._number_of_topics,)
        ...
        hessian_direction_log_nu_square = 1 - numpy.exp(direction_vector * epsilon - log_exp_over_doc_zeta_epsilon_direction + log_exp_over_doc_zeta)
        hessian_direction_log_nu_square *= 0.5 * total_word_count * numpy.exp(doc_log_nu_square - log_exp_over_doc_zeta)
        
        if self._diagonal_covariance_matrix:
            hessian_direction_log_nu_square += 0.5 * (exp_doc_log_nu_square - exp_doc_log_nu_square_epsilon_direction) / self._alpha_sigma;
        else:
            hessian_direction_log_nu_square += 0.5 * (exp_doc_log_nu_square - exp_doc_log_nu_square_epsilon_direction) * numpy.diag(self._alpha_sigma_inv);
        
        hessian_direction_log_nu_square /= epsilon
                
        return hessian_direction_log_nu_square
    
    def hessian_damping_direction_approximation_log_nu_square(self, doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector, damping_factor_initialization=0.1, damping_factor_iteration=10):
        # print "=========="
        ...
        damping_factor_numerator = self.function_log_nu_square(doc_lambda, doc_log_nu_square + direction_vector, doc_zeta_factor, total_word_count);
        damping_factor_numerator -= self.function_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        
        hessian_direction_approximation = self.hessian_direction_approximation_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count, direction_vector);
        
        damping_factor_denominator_temp = self.first_derivative_log_nu_square(doc_lambda, doc_log_nu_square, doc_zeta_factor, total_word_count);
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
        damping_factor_denominator_temp += 0.5 * hessian_direction_approximation;
        assert damping_factor_denominator_temp.shape == (self._number_of_topics,);
